...
class MyWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        cover_label.setGeometry(50,50,600,600)
    ...
    def init_gomoku_ui(self):
        ...
        palette = QPalette()
        palette.setBrush(QPalette.Window, QBrush(QPixmap('imgs/bg.jpg')))
        self.setPalette(palette)
        # 2. 开启鼠标位置的追踪。并在鼠标位置移动时，使用特殊符号标记当前的位置
        self.setMouseTracking(True)
        # 3. 鼠标位置移动时，对鼠标位置的特殊标记
    ...
    def paintEvent(self, e):
        ...
        if hasattr(self, 'game_manager'):  # 游戏还没开始的话，就不用画了
            qp = QPainter()
            qp.begin(self)
            draw_map()  # 绘制棋盘
            draw_pieces()  # 绘制棋子
    ...
    def mouseMoveEvent(self, e):
        ...
        if pos_change and pos_x != -1:
            self.corner_widget.move(25 + pos_x * 40, 25 + pos_y * 40)
    ...
    def mousePressEvent(self, e):
        ...
        if e.button() == Qt.LeftButton:
            ...
            if not judgment:
                return
            # 2. 根据操作结果进行一轮游戏循环
            self.res, self.flash_pieces = self.game_manager.game_state.check_game_over(show=True)  # 判断游戏结果
            if self.res != 0:  # 如果游戏结果为“已经结束”，则显示游戏内容，并退出主循环
                self.repaint(0, 0, 650, 650)
                self.game_restart()
                return
            # 切换角色
            self.game_manager.game_state.switch_player()
    # @run_with_exc
    def end_flash(self):
        # 游戏结束时的闪烁操作
        if self.flash_cnt <= 5:
            ...
            # self.flash_cnt = 0
            # self.game_manager = GameManager()  # 重新初始化游戏内容
